...
from minicode.tui.input_handler import _RawModeContext, _handle_input, _win_read_one_key, _win_has_any_input
        ...
        with _RawModeContext():
            while not should_exit:
                ...
                if sys.platform == "win32":
                    ...
                    has_input = _win_has_any_input() if not has_key else False
                    if not has_key and not has_input:
                        throttled.flush()
                        _time.sleep(0.05)
                        continue
                    chunk = ""

...
class MemoryPipeline:
    ...
    def initialize(
        ...
        if self._memory:
            from minicode.memory_system.injector import MemoryInjector
        ...
        from minicode.memory_system.curator import MemoryCuratorAgent
        ...
        if enable_vector:
            try:
                from minicode.memory_system.vector import SparseVectorStore, VectorMemoryStore
    ...
    def read(
        ...
        """
        # Memory read 是“候选上下文检索”，不是事实裁决。
        # 后续仍应通过 read_file/grep_files 等工具验证当前代码状态。
        if not self._memory:
            return []
        ...
        if self._vector_store and self._vector_store.enabled:
            try:
                ...
                if vec_results:
                    from minicode.memory_system.vector import merge_bm25_vector
    ...
    def inject(
        ...
        """
        # 注入发生在 system prompt 层，且有 cooldown / top limit，
        # 目的是保留项目约定，同时避免 memory 无限制挤占 context。

...
class AnthropicModelAdapter:
    ...
    def next(self, messages: list[dict[str, Any]], on_stream_chunk: Callable[[str], None] | None = None, on_thinking_delta: Callable[[str], None] | None = None, store: Store[AppState] | None = None) -> AgentStep:
        # Adapter 的职责是把内部 ChatMessage 转成 Anthropic Messages API，
        # 再把 text/tool_use/thinking 等 provider-specific block 收敛成 AgentStep。
        ...
        if not on_stream_chunk:
            ...
            if tool_calls:
                # provider-specific 的 tool_use block 到这里就结束，
                # 返回给主循环的只剩统一 toolName/input/id。
                return AgentStep(
                    type="tool_calls",
                    calls=tool_calls,
                    content=parsed_text,

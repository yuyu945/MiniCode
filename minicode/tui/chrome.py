...
def render_banner(
    ...
    """Render the workspace header — compact Claude Code style."""
    t = theme()
    ...
    import os as _os
    cwd_short = _os.path.basename(cwd) if isinstance(cwd, str) else str(cwd)
    return (
        f"{t.subtle}{t.bold}project{t.reset} {t.session}{cwd_short}{t.reset}"
        f"  {t.subtle}model{t.reset} {model}"
        f"  {t.subtle}provider{t.reset} {provider}"
        f"\n"
        f"{t.subtle}{t.bold}session{t.reset}"
        f"  {t.subtle}msgs{t.reset}{msg_count}"
        f" {t.subtle}evts{t.reset}{evt_count}"
        f" {t.subtle}skills{t.reset}{skill_count}"
        f" {t.subtle}mcp{t.reset}{mcp_count}"
    )
...
def render_footer_bar(
    ...
    """Compact single-line footer bar with running tool indicator."""
    t = theme()
    if background_tasks is None:
        background_tasks = []
    width, _ = _cached_terminal_size()
    ...
    if status:
        left = f"{t.tool}{ICON_RUNNING}{t.reset} {t.bold}{status}{t.reset}"
    else:
        left = f"{t.subtle}{ICON_DOT} Ready{t.reset}"

    bg_info = ""
    if background_tasks:
        bg_info = f" {ICON_BG} {t.progress}{len(background_tasks)} bg{t.reset}"

    right = f"{bg_info} {t.subtle}tools{' ✓' if tools_enabled else ' ✗'}{t.reset}"

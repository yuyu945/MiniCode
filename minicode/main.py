...
def main() -> None:
    ...
    # 主架构装配点：不同入口最终都要准备同一组核心对象
    # tools / permissions / model / memory / context，然后交给 run_agent_turn。
    ...
    try:
        if not sys.stdin.isatty():
            for raw_input in sys.stdin:
                ...
                # 非 TTY 模式直接同步跑一轮 agent；TTY 模式会在 input_handler
                # 里用后台线程调用同一个 run_agent_turn。

...
class OpenAIModelAdapter:
    ...
    def next(
        ...
        # Adapter 的职责是把内部 ChatMessage 转成 OpenAI Chat Completions，
        # 再把 OpenAI 的 tool_calls / delta stream 收敛成统一 AgentStep。
        ...
        if not on_stream_chunk:
            ...
            if tool_calls:
                # OpenAI 的 function call 结构不泄漏到 agent_loop；
                # 主循环只看到统一的 toolName/input/id。
                return AgentStep(
                    type="tool_calls",
                    calls=tool_calls,
                    content=parsed_text,
